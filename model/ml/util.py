...
def plot_learning_curve(estimator, title, X, y, scoring="f1_weighted", score_name="F1-weighted", ylim=None, cv=cv_def,
    ...
    return plot_learning_curve_no_comp(train_sizes, train_scores, test_scores, title, score_name, ylim)


def plot_learning_curve_no_comp(train_sizes, train_scores, test_scores, title, score_name="F1-weighted", ylim=None):
    plt.figure()
    plt.title(title)
    if ylim is not None:
      plt.ylim(*ylim)
