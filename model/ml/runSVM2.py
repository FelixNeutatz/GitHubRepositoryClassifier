...
clf = OneVsRestClassifier(SVC(C=10, kernel="linear"))
'''
...
clf = util.fit_cv(svm_rbf, "svm_rbf", train_x, train_y, params)
util.test(clf, "svm_rbf", train_x, train_y, test_x, test_y)
print("Best parameters are %s with a score of %0.2f" % (clf.best_params_, clf.best_score_))
print("Confusion matrix on training data")
util.test(clf, train_x, train_y)
print("Confusion matrix on test data")
...
util.plot_learning_curve(clf, "SVM linear", train_x, train_y, n_jobs=4)
